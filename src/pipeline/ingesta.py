...
class ingestar(luigi.Task):
    ...
    def run(self):
        ...
        if self.tipo_ingesta=='historica':
            ...
            session = boto3.Session(aws_access_key_id=s3_creds['aws_access_key_id'],aws_secret_access_key=s3_creds['aws_secret_access_key'])
            ...
            fecha_ultimo_formato=(datetime.strptime(fecha_ultimo, '%Y-%m-%dT%H:%M:%S.%f')+timedelta(days=1)).date()
            print("########### ", fecha_ultimo)
            print("########### ", fecha_ultimo_formato)
            datos=client.get(constants.id_data_set, where = f"inspection_date between '{fecha_ultimo_formato}' and '{self.fecha}'")
            with self.output().open('w') as outfile:
                pickle.dump(datos,outfile)
                
    #Esto deberá cambiar para que llegue a la EC2 en lugar de a lo local.
    def output(self):
        return luigi.local_target.LocalTarget(f"/home/vivis/Documents/DPA/repo-proyecto/DPA-Chicago-VLIN/data/{self.tipo_ingesta}-{self.fecha}.pkl",format=luigi.format.Nop)